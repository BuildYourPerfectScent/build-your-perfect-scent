...
async function loadMixPool() {
  if (mode === "recommended") {
    ...
    if (storedMixPool.length) {
      ...
      mixHint.textContent = "Choose 2–5 scents from your recommendations.";
      return;
    }
  }
  ...
  try {
    ...
    console.error("Failed to load scents for mix page:", error);
...
function renderMixChoices() {
  if (!mixPool.length) {
    mixPerfumeList.innerHTML = `
      <p class="card-note">No scents available right now.</p>
    `;
    return;
  }

  mixPerfumeList.innerHTML = mixPool.map((p, i) => `
    <label class="mix-choice">
      <input
        type="checkbox"
        class="mix-checkbox"
        id="mix_${i}"
...
function showMixResult() {
  ...
  const ratio = Math.floor(100 / selectedScents.length);
  let remaining = 100 - (ratio * selectedScents.length);

  const blendItems = selectedScents.map((scent, index) => {
    const pct = ratio + (index === 0 ? remaining : 0);
    return { scent, pct };
  });
  ...
  mixResultCard.innerHTML = `
    <div class="mix-result">
      ...
      <p><strong>Your Blend:</strong></p>
      ${blendItems.map(item => `
        <p>${escapeHtml(item.scent)} — ${item.pct}%</p>
      `).join("")}
    </div>
  `;
